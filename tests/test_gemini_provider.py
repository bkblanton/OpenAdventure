...
def test_model_selects_backend_via_settings():
    registry = ModelRegistry.load_default()
    # default (no model set) -> overall default is GPT-6 Sol -> OpenAI backend
...
def test_utility_model_picks_its_own_backend():
    ...
    registry = ModelRegistry.load_default()
    # default utility model is GPT-6 Sol -> OpenAI, independent of the campaign model
...
async def test_cmd_model_switches_backend(make_session, monkeypatch):
    # The default model is GPT-6 Sol (OpenAI); switching to a Gemini model
    # flips the backend.
